	...
	public class PornSqlRepository : BaseSqlRepository<Porn> {
		...
		public override List<Porn> GetList() {
			using (var connection = new SqlConnection(_connectionString)) {
				var command = new SqlCommand("SELECT a.firstname, a.lastname, p.name, p.pornstar, p.duration, p.rating FROM Porn AS p INNER JOIN Authors AS a ON p.authorId = a.authorId", connection);
